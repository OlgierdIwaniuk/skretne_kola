...
float count=0.0f;
float angle;
float przełożenie=100.0f;
extern float receivedInt;
float Kp = 1.0f;
float Ki = 0.05f;
float Kd = 0.1f;
...
float integral = 0.0f;
uint32_t current_pwm = 0;  // Aktualna wartość PWM (po rampowaniu)
uint32_t ramp_step = 5;    // Maksymalna zmiana PWM na jedną iterację
uint32_t pwm_output = 0;
/* USER CODE END PV */
